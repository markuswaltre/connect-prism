...
function MockFilenameGenerator(prismUtils) {
   ...
   * setting.
   */
  function defaultMockFilename(config, req, status) {
  ...
  function humanReadableMockFilename(config, req, status) {
    var maxLength = 255;
  ...
  this.getMockPath = function(prism, req, status, count) {
    var fn;
    ...
    var responseFilename = fn(prism.config, req, status, count);
