...
// and logging
function Mock(logger, prismUtils, mockFilenameGenerator, responseDelay, urlCounter) {

  function getMockPath(req, res, prism) {
    var url = prismUtils.filterUrl(prism.config, req.url);
  ...
  this.serializeOverrideMock = function(prism, override) {
    ...
    var status = override.mock.statusCode;
    var path = mockFilenameGenerator.getMockPath(prism, req, status);
...
di.annotate(Mock, new di.Inject(Logger, PrismUtils, MockFilenameGenerator, ResponseDelay, UrlCounter));
